...
Bilbycast wrapper over the EBU / Linux Foundation **Media eXchange Layer (MXL)** Rust crate (`dmf-mxl/mxl` v1.0.1, Apache-2.0). Used only when bilbycast-edge is built with `--features mxl` (default off).
...
```bash
# 1. Clone with submodules (vendored upstream MXL pinned to v1.0.1)
